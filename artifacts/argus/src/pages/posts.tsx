...
function usePosts() {
  return useQuery<SavedPost[]>({
    queryKey: ["posts"],
    queryFn: async () => { const r = await fetch(`${BASE}/api/posts`); return r.json(); },
  });
}
...
function useSavePost() {
  ...
  return useMutation<SavedPost, Error, Omit<SavedPost, "id" | "createdAt">>({
    mutationFn: async (data) => {
      const r = await fetch(`${BASE}/api/posts`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) });
      return r.json();
    },
...
function PlatformBadge({ platform }: { platform: string }) {
  ...
  const Icon = cfg.icon;
  return <div className={`flex items-center gap-1 text-[11px] font-semibold ${cfg.color}`}><Icon className="w-3 h-3" /> {cfg.label}</div>;
}
...
function CopyButton({ text, className = "" }: { text: string; className?: string }) {
  ...
  return (
    <button onClick={doCopy}
      className={`flex items-center gap-1.5 text-xs font-medium rounded-lg px-2.5 py-1.5 transition-all ${copied ? "bg-green-500/10 text-green-500" : "bg-muted hover:bg-muted/80 text-muted-foreground hover:text-foreground"} ${className}`}
    >
      ...
      <div className="p-5 space-y-5">
            ...
            {PLATFORMS.map((p) => {
              ...
              return (
                <button key={p.id} onClick={() => setPlatform(p.id)}
                  className={`flex items-center gap-3 px-3.5 py-2.5 rounded-xl border text-sm font-medium transition-all ${active ? "border-primary/40 bg-primary/8 text-foreground shadow-sm" : "border-border bg-card hover:bg-muted/50 text-muted-foreground hover:text-foreground"}`}
                >
          ...
          <div className="flex flex-wrap gap-1.5">
            {TONES.map((t) => (
              <button key={t.id} onClick={() => setTone(t.id)}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${tone === t.id ? "bg-primary text-primary-foreground border-primary shadow-sm" : "bg-card border-border text-muted-foreground hover:text-foreground hover:border-primary/40"}`}
              >
          ...
          <div className="grid grid-cols-2 gap-2">
            {(["short", "long"] as const).map((l) => (
              <button key={l} onClick={() => setLength(l)}
                className={`py-2 rounded-xl text-sm font-medium border transition-all ${length === l ? "bg-primary text-primary-foreground border-primary" : "bg-card border-border text-muted-foreground hover:border-primary/40"}`}
              >
        ...
        <div>
          ...
          <textarea
            ...
            rows={4}
            className="w-full rounded-xl border border-input bg-card px-3.5 py-3 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 resize-none"
          />
          <p className="text-[11px] text-muted-foreground mt-1.5">⌘ + Enter to generate</p>
        </div>

        <button onClick={onGenerate} disabled={!topic.trim() || isPending}
          className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-primary text-primary-foreground text-sm font-semibold disabled:opacity-40 hover:bg-primary/90 transition-all shadow-sm active:scale-[0.98]"
        >
      ...
      <div className="p-5 max-w-lg mx-auto">
        {isPending ? (
          <div className="space-y-4 animate-pulse">
            <div className="rounded-2xl border border-border bg-card p-5 space-y-3">
              <div className="flex items-center gap-3">
                ...
                <div className="space-y-1.5"><div className="h-3 w-24 bg-muted rounded-full" /><div className="h-2.5 w-16 bg-muted rounded-full" /></div>
              </div>
              <div className="space-y-2 pt-1">
                {[...Array(4)].map((_, i) => <div key={i} className="h-3 bg-muted rounded-full" style={{ width: `${90 - i * 10}%` }} />)}
              </div>
            </div>
          ...
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-3 duration-300">
            <div className="rounded-2xl border border-border bg-card overflow-hidden shadow-sm">
              <div className="flex items-center gap-3 px-4 pt-4 pb-3 border-b border-border/50">
                <div className="w-9 h-9 rounded-full bg-gradient-to-br from-primary/60 to-primary flex items-center justify-center text-white text-sm font-bold shrink-0">A</div>
                <div>
              ...
              <div className="px-4 py-3">
                {generated.hook && <p className="text-sm font-semibold mb-2">{generated.hook}</p>}
                ...
                {generated.hashtags?.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-3">
                    {generated.hashtags.map((tag: string, i: number) => (
                      <span key={i} className="text-xs text-primary font-medium">#{tag.replace(/^#/, "")}</span>
                    ))}
                  </div>
                )}
              </div>
            ...
            <div className="flex gap-2">
              {savedId ? (
                <div className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl border border-green-500/30 bg-green-500/5 text-green-500 text-sm font-medium">
                  <Check className="w-4 h-4" /> Saved to library
                </div>
              ) : (
                <button onClick={onSave} disabled={isSaving}
                  className="flex-1 py-3 rounded-xl border border-primary/40 text-primary text-sm font-semibold hover:bg-primary/5 transition-all"
                >
                  {isSaving ? "Saving…" : "Save to Library"}
                </button>
              )}
              <button onClick={onRegenerate} disabled={isPending}
                className="px-4 py-3 rounded-xl bg-muted hover:bg-muted/80 text-sm font-medium transition-all flex items-center gap-1.5"
              >
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center text-muted-foreground">
            ...
            <p className="text-sm max-w-xs opacity-70">Pick a platform, choose your tone, describe your topic, and let Argus write it.</p>
          </div>
        )}
      </div>
    </ScrollArea>
    ...
    <ScrollArea className="flex-1">
      <div className="p-3 space-y-2">
        {posts.length === 0 ? (
          ...
          return (
            <div key={post.id} className="group rounded-xl border border-border bg-card p-3 hover:border-primary/30 transition-all">
              ...
              <p className="text-xs text-muted-foreground mb-1.5 truncate">{post.topic}</p>
              <p className="text-sm leading-relaxed line-clamp-3">{post.content}</p>
              {tags.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-2">
                  {tags.slice(0, 4).map((t, i) => <span key={i} className="text-[10px] text-primary">#{t.replace(/^#/, "")}</span>)}
    ...
    return (
      <div className="flex flex-col h-full bg-background">
        ...
        <div className="px-4 pt-5 pb-0 border-b border-border">
          <h1 className="text-xl font-bold mb-3">Social Posts</h1>
